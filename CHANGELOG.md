...
- Scan failures caused by macOS privacy protection (for example reading `~/.Trash`
  without Full Disk Access) now offer a direct **Open Full Disk Access…** button
  next to Retry, so the permission fix is one click away.
